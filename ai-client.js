...
async function apiPost(path,body){if(!API_BASE)throw new Error('AI 服务地址尚未配置');const r=await fetch(`${API_BASE}${path}`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});const data=await r.json().catch(()=>({}));if(!r.ok||!data.ok)throw new Error(data.message||`服务请求失败（${r.status}）`);return data.data}
const lookupBook=bookName=>apiPost('/api/lookup',{bookName});
const generateFromSource=(bookName,audience,sourceText,sourceType)=>apiPost('/api/generate',{bookName,audience,sourceText,sourceType});
const generateFromWeb=(bookName,audience)=>apiPost('/api/web-search',{bookName,audience,userConsent:true});
